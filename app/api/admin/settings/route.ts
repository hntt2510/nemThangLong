...
const schema = z.object({ shippingFee: z.number().int().min(0).nullable(), freeShippingThreshold: z.number().int().min(0).nullable(), bankTransferInfo: z.record(z.string()).nullable() });
...
export async function PUT(request: Request) {
  ...
  const prisma = getPrisma();
  if (!prisma) return NextResponse.json({ error: "Database chưa được cấu hình." }, { status: 503 });
  const data = { shippingFee: parsed.data.shippingFee, freeShippingThreshold: parsed.data.freeShippingThreshold, bankTransferInfo: parsed.data.bankTransferInfo ? parsed.data.bankTransferInfo as Prisma.InputJsonValue : Prisma.JsonNull };
