...
import { confirmBankTransfer, confirmCodCollected, cancelOrder } from "@/lib/payment-lifecycle";
import { adminOrderActionSchema, getAdminOrder, updateFulfillmentStatus } from "@/lib/admin-orders";

export async function GET(_: Request, context: { params: Promise<{ id: string }> }) {
  const status = authStatus(await auth(), ["ADMIN"]); if (status !== 200) return NextResponse.json({ error: status === 401 ? "Unauthorized" : "Forbidden" }, { status });
...
export async function PATCH(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if ("action" in body && body.action === "confirm_cod_paid") { const order = await confirmCodCollected(prisma, id); return NextResponse.json({ id: order.id, status: order.status, paymentStatus: order.paymentStatus }); }
