...
export async function POST(request: Request) {
  ...
  const media = await prisma.$transaction(async (tx) => {
    ...
    const asset = await tx.mediaAsset.create({ data: { ...buildFinalizedMediaData({ productId: body.productId!, type: intent.kind, url: `${config.env.R2_PUBLIC_URL}/${intent.key}`, alt: body.alt!, aspect: body.aspect }), storageKey: intent.key, mimeType: intent.contentType, sourceType: "UPLOAD", reviewStatus: "DRAFT" } });
    await tx.productMedia.create({ data: { productId: body.productId!, mediaAssetId: asset.id, role: "gallery", sortOrder: 0 } });
    return asset;
  });
