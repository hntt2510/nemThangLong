...
export default async function SleepJournalArticlePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <>
      <SiteHeader solid />
      <main className="container journal-article">
        <nav className="journal-breadcrumb" aria-label="Breadcrumb">
          <Link href="/">Trang chủ</Link>
          <span aria-hidden="true">/</span>
          <Link href="/kien-thuc-giac-ngu">Blog</Link>
        </nav>
        <p className="eyebrow">BLOG THĂNG LONG</p>
        <h1>{post.title}</h1>
        <div className="journal-article-image">
          <Image src={post.image} alt="" fill priority sizes="(max-width: 860px) 100vw, 720px" />
        </div>
        ...
        <section className="journal-related">
          ...
          <div>
            {related.map((item) => (
              <Link key={item.slug} href={`/kien-thuc-giac-ngu/${item.slug}` as never}>
                {item.title} <span aria-hidden="true">→</span>
              </Link>
            ))}
          </div>
        </section>
        <section className="journal-consult">
          <div>
            <h2>Cần tư vấn thêm?</h2>
            <p>Liên hệ để trao đổi theo nhu cầu và không gian phòng ngủ của bạn.</p>
          </div>
          <a href={contactHref} className="button button-primary">
