...
export default async function AdminSettingsPage() {
  ...
  return <main className="admin-placeholder"><Link href="/admin">← Product editor</Link><p className="eyebrow">ADMIN / SETTINGS</p><h1>Site settings</h1><p>Checkout chỉ mở khi phí vận chuyển và phương thức thanh toán đã được cấu hình.</p><AdminSettingsForm initial={settings ? { shippingFee: settings.shippingFee, freeShippingThreshold: settings.freeShippingThreshold, bankTransferInfo: settings.bankTransferInfo as Record<string, unknown> | null } : null} /></main>;
}
