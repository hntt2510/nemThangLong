...
export default async function PaymentReviewDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const item = await getPaymentReview(prisma, (await params).id).catch(() => null); if (!item) notFound(); const payment = item.payments[0];
  return <main className="admin-placeholder"><Link href="/admin/payment-reviews">← Payment review</Link><p className="eyebrow">PAYMENT REVIEW / {item.code}</p><h1>{item.customerName}</h1><p>{item.total.toLocaleString("vi-VN")} ₫ · {payment?.providerTransactionId ?? "—"}</p><p className="muted">Order tạo lúc {item.createdAt.toLocaleString("vi-VN")}</p><section className="dashboard-section"><h2>Payment context</h2><dl><dt>Attempt status</dt><dd>{payment?.status ?? "—"}</dd><dt>Expires at</dt><dd>{payment?.expiresAt?.toLocaleString("vi-VN") ?? "—"}</dd><dt>Received / updated at</dt><dd>{payment?.updatedAt.toLocaleString("vi-VN") ?? "—"}</dd></dl></section><div className="dashboard-section"><h2>Items và tồn kho hiện tại</h2><ul>{item.items.map((line) => <li key={line.id}><span>{line.productName} · {line.sku} · {line.quantity} · tồn hiện tại {line.variant.stock}</span><strong>{item.reservations.filter((reservation) => reservation.variantId === line.variantId).map((reservation) => `${reservation.status} × ${reservation.quantity}${reservation.releasedAt ? ` · released ${reservation.releasedAt.toLocaleString("vi-VN")}` : ""}`).join(", ") || "Đã release"}</strong></li>)}</ul></div><p className="admin-note">Chỉ ADMIN được xử lý. Fulfill sẽ kiểm tra tồn kho và trừ atomically; manual refund chỉ ghi nhận hoàn tiền đã thực hiện ngoài hệ thống.</p><AdminPaymentReviewActions orderId={item.id} /></main>;
}
