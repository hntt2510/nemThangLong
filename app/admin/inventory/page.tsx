...
export default async function InventoryPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return <main className="admin-placeholder"><Link href="/admin/products">← Products</Link><p className="eyebrow">ADMIN / INVENTORY</p><h1>Tồn kho</h1><p className="muted">Điều chỉnh theo delta; không có ngưỡng tồn kho tự đặt.</p><div className="admin-order-list">{result.items.length === 0 ? <p className="admin-note">Chưa có variant.</p> : result.items.map((item) => <article className="admin-order-card" key={item.id}><div><strong>{item.product.name}</strong><span>{item.product.slug} · {item.sku} · {item.width}×{item.length}×{item.thickness}</span></div><div><strong>{item.stock}</strong><span>{item.active ? "Active" : "Inactive"}</span><small>{item.inventoryAdjustments[0] ? `Điều chỉnh gần nhất ${item.inventoryAdjustments[0].delta > 0 ? "+" : ""}${item.inventoryAdjustments[0].delta}` : "Chưa có điều chỉnh thủ công"}</small></div></article>)}</div></main>;
}
