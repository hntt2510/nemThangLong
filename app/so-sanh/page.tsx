...
import { isUiShowcaseMode, SHOWCASE_COMPARE_DEFAULT_ITEMS } from "@/lib/ui-showcase";

export default async function ComparePage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  const params = await searchParams;
  const [{ products }, settings] = await Promise.all([getDiscoveryProducts(), getSiteSettings()]);
  const parsedItems = parseCompareItems(params);
  const items = parsedItems.length > 0 ? parsedItems : isUiShowcaseMode() ? SHOWCASE_COMPARE_DEFAULT_ITEMS : [];
