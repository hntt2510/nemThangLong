...
export function MattressLabViewer({ product }: { product: Product }) {
  ...
  const fallback = <div className="viewer-fallback"><img src={product.posterUrl ?? product.media[0]?.url} alt="Poster Mattress Lab — mô hình 3D sẽ xuất hiện sau khi được cấu hình" /><div><p className="eyebrow">MATTRESS LAB {product.isDemo ? "· 3D DEMO" : ""}</p><h2>{product.isDemo ? "3D demo — không phải cấu tạo sản phẩm" : "Khám phá cấu tạo"}</h2><p>{product.isDemo ? "Mô hình minh họa chỉ dùng để kiểm tra trải nghiệm viewer; không đại diện cho thông số sản phẩm." : "Mô hình 3D hiển thị chi tiết các tầng nâng đỡ của sản phẩm."}</p></div></div>;
