...
type Result = { code: string; paymentMethod: "COD" | "BANK_TRANSFER" | "MOMO"; paymentStatus: "PENDING" | "PAID" | "FAILED" | "REVIEW_REQUIRED" | "REFUNDED"; status: string; paymentExpiresAt: string | null; total: number; items: Array<{ name: string; quantity: number; width: number; length: number; thickness: number }>; bankTransferInfo: Record<string, unknown> | null };
...
export function OrderResult({ token }: { token: string }) {
  ...
  const failed = result.paymentStatus === "FAILED" || result.status === "CANCELLED"; const review = result.paymentStatus === "REVIEW_REQUIRED";
  return <main className="success-page"><p className="eyebrow">{success ? "ĐẶT HÀNG THÀNH CÔNG" : review ? "CẦN ĐỐI SOÁT THANH TOÁN" : failed ? "THANH TOÁN CHƯA THÀNH CÔNG" : "ĐƠN HÀNG ĐANG CHỜ XÁC NHẬN"}</p><h1>{success ? "Cảm ơn bạn đã chọn Thăng Long." : review ? "MoMo đã báo thanh toán nhưng đơn hàng cần được đối soát thủ công." : failed ? "Giao dịch chưa hoàn tất." : result.paymentMethod === "BANK_TRANSFER" ? "Vui lòng hoàn tất chuyển khoản." : "Đang chờ MoMo xác nhận."}</h1><p>Mã đơn hàng: <strong>{result.code}</strong> · Tổng: <strong>{formatVnd(result.total)}</strong></p>{result.paymentMethod === "BANK_TRANSFER" && result.paymentExpiresAt && <p>Hạn giữ tồn kho: <strong>{new Date(result.paymentExpiresAt).toLocaleString("vi-VN")}</strong></p>}{result.items.map((item) => <p key={`${item.name}-${item.width}-${item.thickness}`} className="order-result-item">{item.name} · {formatDimension(item.width)} × {formatDimension(item.length)} × {item.thickness}cm · {item.quantity}</p>)}{result.paymentMethod === "BANK_TRANSFER" && result.bankTransferInfo && <pre className="bank-transfer-info">{JSON.stringify(result.bankTransferInfo, null, 2)}</pre>}<Link href="/nem/luxury" className="button button-primary">Quay lại Luxury</Link></main>;
}
