    ...
    <header ref={menuRef} className={"site-header " + (scrolled ? "is-scrolled" : "")}>
      <div className="header-inner">
        ...
        <nav id="mobile-navigation" className={"desktop-nav " + (mobileOpen ? "is-mobile-open" : "")} aria-label="Điều hướng chính">
          <button id="mattress-menu-trigger" className="nav-link has-menu" type="button" onClick={() => dispatchMenu((state) => reduceHeaderMenu(state, { type: "toggle-mega" }))} aria-expanded={megaOpen} aria-controls="mattress-menu">Nệm <span aria-hidden="true">⌄</span></button>
          ...
          {megaOpen && <div id="mattress-menu" className="mega-menu" role="region" aria-label="Khám phá nệm">
            <div><p className="eyebrow">NỆM</p><h2>Chọn cảm giác phù hợp.</h2><p className="muted">Khám phá theo dòng nệm hoặc nhu cầu ngủ của bạn.</p><Link href={"/nem" as never} className="text-link" onClick={closeMenu}>Xem tất cả nệm <span aria-hidden="true">→</span></Link></div>
        ...
        <div className="header-actions">
          ...
          <button className="menu-toggle" type="button" onClick={() => dispatchMenu((state) => reduceHeaderMenu(state, { type: "toggle-mobile" }))} aria-label={mobileOpen ? "Đóng menu" : "Mở menu"} aria-expanded={mobileOpen} aria-controls="mobile-navigation">☰</button>
        </div>
      </div>
    </header>
  );
