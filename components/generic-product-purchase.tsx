  ...
  return (
    <section className="generic-product-purchase container">
      ...
      <div className="generic-product-copy">
        ...
        {variants.length > 0 ? <div className="variant-block"><div className="variant-heading"><span>KÍCH THƯỚC</span><small>{selected ? formatDimension(selected.width) + " × " + formatDimension(selected.length) + " × " + selected.thickness + "cm" : "Đang cập nhật"}</small></div>{(["width", "length", "thickness"] as const).map((dimension) => <label key={dimension}>{dimension === "width" ? "Rộng" : dimension === "length" ? "Dài" : "Độ dày"}<select value={selected?.[dimension] ?? ""} onChange={(event) => change(dimension, Number(event.target.value))} aria-label={dimension === "width" ? "Chiều rộng" : dimension === "length" ? "Chiều dài" : "Độ dày"}>{dimensionOptions(variants, dimension, selected).map((value) => <option key={value} value={value}>{formatDimension(value)}</option>)}</select></label>)}</div> : <div className="variant-placeholder">Kích thước và tổ hợp bán đang được cập nhật từ CMS.</div>}
        {selected && <p className="generic-availability" aria-live="polite">{selected.stock > 0 ? "Còn hàng" : "Tạm hết hàng"}{selected.price === null || selected.price <= 0 ? " · Giá đang cập nhật" : ""}</p>}
        {canPurchase ? <div className="hero-ctas"><button className="button button-primary" type="button" onClick={buyNow}>Mua ngay</button><button className="button button-secondary" type="button" onClick={addToCart}>Thêm vào giỏ</button></div> : contactHref ? <a className="button button-primary contact-button" href={contactHref}>Liên hệ tư vấn</a> : <p className="generic-contact-pending">Thông tin tư vấn đang được cập nhật.</p>}
        <div className="trust-list"><span>✓ Thông tin sản phẩm theo cấu hình CMS</span><span>✓ Không hiển thị tổ hợp chưa được xác nhận</span></div>
      </div>
    </section>
  );
}
