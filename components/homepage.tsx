  ...
  return <div className="homepage">
    <main>
      <section className="home-hero">
        <div className="container home-hero-grid">
          <div className="home-hero-copy">
            ...
            <div className="home-hero-actions"><Link href={"/tim-nem" as never} className="button button-primary">Tìm nệm phù hợp</Link><Link href="#product-range" className="button button-secondary">Khám phá sản phẩm</Link></div>
      ...
      <GsapReveal><section id="find-mattress" className="home-find container">
        <div className="home-find-intro"><p className="section-label">FIND YOUR MATTRESS</p><h2>Bắt đầu từ cách bạn muốn được nghỉ ngơi.</h2><p>Trả lời vài câu hỏi về kích thước và cảm giác bạn tìm kiếm để xem những lựa chọn có dữ liệu phù hợp.</p><Link href={"/tim-nem" as never} className="text-link">Bắt đầu tìm nệm <span aria-hidden="true">→</span></Link></div>
      ...
      <GsapReveal><section id="natural-latex" className="home-latex container"><div className="home-latex-copy"><p className="section-label">NATURAL LATEX</p><h2>Một câu chuyện về cảm giác tự nhiên.</h2><p>{latex.materialStory?.body ?? "Nội dung về Cao Su Thiên Nhiên đang được cập nhật từ CMS. Hình ảnh chỉ mang tính minh họa và không đại diện cho cấu tạo sản phẩm."}</p><Link href="#product-range" className="text-link">Xem dòng Cao Su Thiên Nhiên <span aria-hidden="true">→</span></Link></div><div className="home-latex-media"><Image src="/images/homepage-natural-latex.webp" alt="Hình ảnh minh họa nguồn cao su tự nhiên" fill sizes="(max-width: 860px) 100vw, 58vw" /><span className="demo-badge">Hình ảnh minh họa</span></div></section></GsapReveal>

      <GsapReveal><section id="shop-by-need" className="home-needs container"><div className="home-needs-intro"><p className="section-label">SHOP BY NEED</p><h2>Chọn theo điều bạn cần mỗi đêm.</h2><p>Những gợi ý ban đầu để bạn bắt đầu với Finder. Mọi trường dữ liệu chưa được xác nhận sẽ được ghi rõ.</p><Link href={"/tim-nem" as never} className="text-link">Tìm nệm theo nhu cầu <span aria-hidden="true">→</span></Link></div><div className="home-needs-list">{[["01", "Êm ái", "Cho cảm giác mềm mại, thư giãn."], ["02", "Nâng đỡ", "Cho lựa chọn cân bằng và vững vàng."], ["03", "Ngủ mát", "Cho không gian nghỉ ngơi thoáng đãng."], ["04", "Cặp đôi & gia đình", "Cho những nhu cầu ngủ cùng nhau."]].map(([number, title, body]) => <Link key={number} href={"/tim-nem" as never}><span>{number}</span><strong>{title}</strong><small>{body}</small><b aria-hidden="true">↗</b></Link>)}</div></section></GsapReveal>

      <GsapReveal><section id="compare" className="home-compare container"><div><p className="section-label">COMPARE</p><h2>Chưa chắc lựa chọn nào hợp với bạn?</h2><p>Đặt cạnh các dòng nệm bằng dữ liệu đã được công bố.</p><Link href={"/so-sanh" as never} className="button button-secondary">Mở trang so sánh</Link></div><div className="home-compare-lines"><span>Luxury</span><span>Memory Foam</span><span>Cao Su Thiên Nhiên</span><span>America</span></div></section></GsapReveal>

      <GsapReveal><section id="hotel-project" className="home-project container"><div className="home-project-media"><Image src="/images/homepage-hotel.webp" alt="Hình ảnh minh họa phòng ngủ khách sạn" fill sizes="(max-width: 860px) 100vw, 58vw" /><span className="demo-badge">Hình ảnh minh họa</span></div><div className="home-project-copy"><p className="section-label">HOTEL &amp; PROJECT</p><h2>Giấc ngủ tốt, ở mọi quy mô.</h2><p>Thông tin dành cho khách sạn và dự án đang được chuẩn bị. Hãy trao đổi nhu cầu thực tế với đội ngũ Thăng Long.</p><Link href={"/khach-san-du-an" as never} className="text-link">Khám phá khách sạn &amp; dự án <span aria-hidden="true">→</span></Link></div></section></GsapReveal>
      ...
      <section id="contact" className="home-consultation"><div className="container"><p className="eyebrow">TƯ VẤN LỰA CHỌN</p><h2>Bắt đầu bằng một câu hỏi đơn giản.</h2><p>Chúng tôi sẽ lắng nghe cách bạn ngủ trước khi gợi ý một lựa chọn.</p><Link className="button button-dark" href={"/lien-he" as never}>Liên hệ tư vấn <span aria-hidden="true">→</span></Link>{contactHref && <a className="home-direct-contact" href={contactHref}>Hoặc liên hệ trực tiếp</a>}</div></section>
    </main>
