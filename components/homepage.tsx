...
export function Homepage({ products, luxuryProduct, settings }: { products: HomepageProductSummary[]; luxuryProduct: Product; settings: HomepageSettings | null }) {
  ...
  return <div className="homepage">
    <main>
      ...
      <GsapReveal><section id="shop-by-need" className="home-needs container"><div className="home-needs-intro"><p className="section-label">SHOP BY NEED</p><h2>Chọn theo điều bạn cần mỗi đêm.</h2><p>Những gợi ý ban đầu để bạn trò chuyện với đội ngũ tư vấn. Công cụ Finder đầy đủ sẽ được bổ sung sau.</p></div><div className="home-needs-list">{[["01", "Êm ái", "Cho cảm giác mềm mại, thư giãn."], ["02", "Nâng đỡ", "Cho lựa chọn cân bằng và vững vàng."], ["03", "Ngủ mát", "Cho không gian nghỉ ngơi thoáng đãng."], ["04", "Cặp đôi & gia đình", "Cho những nhu cầu ngủ cùng nhau."]].map(([number, title, body]) => contactHref ? <Link key={number} href="#contact"><span>{number}</span><strong>{title}</strong><small>{body}</small><b aria-hidden="true">↗</b></Link> : <div key={number} className="home-needs-item"><span>{number}</span><strong>{title}</strong><small>{body}</small><em>Thông tin đang cập nhật</em></div>)}</div></section></GsapReveal>

      <GsapReveal><section id="compare" className="home-compare container"><div><p className="section-label">COMPARE</p><h2>Chưa chắc lựa chọn nào hợp với bạn?</h2><p>So sánh đầy đủ sẽ được xây dựng sau. Hiện tại, đội ngũ tư vấn có thể giúp bạn bắt đầu từ nhu cầu thực tế.</p>{contactHref ? <Link href="#contact" className="button button-secondary">Nhận tư vấn lựa chọn</Link> : <p className="home-contact-pending">Thông tin tư vấn đang được cập nhật.</p>}</div><div className="home-compare-lines"><span>Luxury</span><span>Memory Foam</span><span>Cao Su Thiên Nhiên</span><span>America</span></div></section></GsapReveal>
