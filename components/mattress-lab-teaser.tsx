...
export function MattressLabTeaser({ product }: { product: Product }) {
  ...
  return <section id="mattress-lab" ref={scope} className="lab-section"><div className="lab-copy"><p className="eyebrow">MATTRESS LAB</p><h2>{demo ? <>3D DEMO<br />VIEWER<br />PREVIEW.</> : <>EXPLORE<br />THE<br />PRODUCT.</>}</h2><p>{demo ? "Mô hình minh họa để kiểm tra trải nghiệm viewer; không phải cấu tạo sản phẩm." : "Khám phá các lớp đã được xác nhận trong CMS."}</p><Link href="/nem/luxury/lab" className="button button-dark">Khám phá Mattress Lab <span aria-hidden="true">→</span></Link></div><div className="lab-visual"><div className="lab-poster"><img src={product.posterUrl ?? product.media[0]?.url} alt={demo ? "Hình ảnh minh họa Mattress Lab" : "Poster Mattress Lab"} />{nearViewport && <span className="lab-badge">{demo ? "3D DEMO" : "3D READY"}</span>}</div><span className="lab-hint">↻ Drag to rotate</span></div></section>;
}
