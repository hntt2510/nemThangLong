...
export function FinderResultsPanel({ results }: { results: FinderResults }) {
  if (results.empty) {
    return <section id="results" className="finder-results container" aria-live="polite">
      <div className="finder-empty">
        <p className="section-label">KẾT QUẢ</p>
        <h2>Chưa có sản phẩm với dữ liệu đã xác nhận phù hợp các điều kiện này.</h2>
        <div className="finder-result-actions"><Link href="/tim-nem#finder-form" className="button button-secondary">Điều chỉnh điều kiện</Link><Link href="/nem" className="button button-primary">Xem tất cả nệm</Link></div>
      </div>
    </section>;
  }
  return (
