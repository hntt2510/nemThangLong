...
export const OPENAPI_ROUTE_INVENTORY = [
  ...
  ["GET", "/api/admin/after-sales/[id]"], ["PATCH", "/api/admin/after-sales/[id]"], ["GET", "/api/admin/after-sales"], ["GET", "/api/admin/categories"], ["POST", "/api/admin/categories"], ["GET", "/api/admin/categories/[id]"], ["PATCH", "/api/admin/categories/[id]"], ["DELETE", "/api/admin/categories/[id]"], ["GET", "/api/admin/dashboard"], ["GET", "/api/admin/data-quality"], ["GET", "/api/admin/inventory"], ["POST", "/api/admin/inventory"], ["GET", "/api/admin/inventory/[variantId]/history"], ["GET", "/api/admin/leads/[id]"], ["PATCH", "/api/admin/leads/[id]"], ["GET", "/api/admin/leads"], ["GET", "/api/admin/media"], ["GET", "/api/admin/orders/[id]"], ["PATCH", "/api/admin/orders/[id]"], ["GET", "/api/admin/orders"], ["GET", "/api/admin/payment-reviews/[id]"], ["PATCH", "/api/admin/payment-reviews/[id]"], ["GET", "/api/admin/payment-reviews"], ["GET", "/api/admin/products/[slug]"], ["PUT", "/api/admin/products/[slug]"], ["GET", "/api/admin/products"], ["POST", "/api/admin/products"], ["GET", "/api/admin/reviews"], ["POST", "/api/admin/reviews"], ["PATCH", "/api/admin/reviews/[id]"], ["DELETE", "/api/admin/reviews/[id]"], ["GET", "/api/admin/settings"], ["PUT", "/api/admin/settings"], ["GET", "/api/admin/sources"], ["POST", "/api/admin/sources"], ["PATCH", "/api/admin/sources/[id]"], ["GET", "/api/admin/users"],
  ["POST", "/api/auth/register"], ["POST", "/api/chat"], ["POST", "/api/checkout"], ["GET", "/api/cron/release-reservations"], ["POST", "/api/leads"], ["GET", "/api/orders/result/[token]"], ["POST", "/api/payments/sepay/webhook"], ["GET", "/api/products"], ["GET", "/api/products/[slug]"], ["GET", "/api/storefront/settings"], ["POST", "/api/uploads/finalize"], ["POST", "/api/uploads/presign"],
...
const variantIdParam = z.object({ variantId: z.string().min(1) }).strict();
const paginationQuery = z.object({ page: z.coerce.number().int().positive().optional(), pageSize: z.coerce.number().int().positive().max(100).optional() }).strict();
...
const categoryInput = z.object({ slug: z.string().trim().regex(/^[a-z0-9]+(?:-[a-z0-9]+)*$/).max(120), name: z.string().trim().min(1).max(160), description: z.string().trim().max(5000).nullable().optional(), status: z.enum(["DRAFT", "PUBLISHED"]).default("DRAFT"), sortOrder: z.number().int().min(0).default(0) }).strict();
const reviewInput = z.object({ productId: z.string().min(1), authorName: z.string().trim().min(1).max(120), rating: z.number().int().min(1).max(5), body: z.string().trim().min(1).max(4000), approved: z.boolean().default(false) }).strict();
const reviewUpdateInput = z.object({ approved: z.boolean() }).strict();
const userSearchQuery = z.object({ q: z.string().max(100).optional() }).strict();
const chatPayloadSchema = z.object({ messages: z.array(z.object({ role: z.enum(["user", "assistant", "system"]), content: z.string().min(1) })).min(1) }).strict();
const publicCatalogQuery = z.object({ q: z.string().max(100).optional(), sort: z.enum(["featured", "price-asc", "price-desc", "name-asc"]).optional(), line: z.string().optional(), width: z.coerce.number().int().positive().optional(), thickness: z.coerce.number().int().positive().optional(), minPrice: z.coerce.number().int().nonnegative().optional(), maxPrice: z.coerce.number().int().nonnegative().optional(), inStock: z.enum(["0", "1"]).optional(), page: z.coerce.number().int().positive().optional(), limit: z.coerce.number().int().positive().max(100).optional() }).strict();
...
register({ method: "get", path: "/api/admin/categories", tags: ["Admin Products"], operationId: "listAdminCategories", ...editor, responses: { "200": response(anyArray), ...errors([401, 403, 503]) } });
register({ method: "post", path: "/api/admin/categories", tags: ["Admin Products"], operationId: "createAdminCategory", ...editor, request: { body: body(categoryInput) }, responses: { "201": response(anyObject), ...errors([400, 401, 403, 409, 503]) } });
register({ method: "get", path: "/api/admin/categories/{id}", tags: ["Admin Products"], operationId: "getAdminCategory", ...editor, request: { params: pathId }, responses: { "200": response(anyObject), ...errors([401, 403, 404, 503]) } });
register({ method: "patch", path: "/api/admin/categories/{id}", tags: ["Admin Products"], operationId: "updateAdminCategory", ...editor, request: { params: pathId, body: body(categoryInput.partial()) }, responses: { "200": response(anyObject), ...errors([400, 401, 403, 404, 503]) } });
register({ method: "delete", path: "/api/admin/categories/{id}", tags: ["Admin Products"], operationId: "deleteAdminCategory", ...admin, request: { params: pathId }, responses: { "204": { description: "Category deleted" }, ...errors([401, 403, 409, 503]) } });
...
register({ method: "get", path: "/api/admin/inventory/{variantId}/history", tags: ["Inventory"], operationId: "getInventoryVariantHistory", ...admin, request: { params: variantIdParam, query: paginationQuery }, responses: { "200": response(anyObject), ...errors([401, 403, 404, 503]) } });
...
register({ method: "get", path: "/api/admin/media", tags: ["Uploads"], operationId: "listAdminMedia", ...editor, responses: { "200": response(anyArray), ...errors([401, 403, 503]) } });
...
register({ method: "get", path: "/api/admin/reviews", tags: ["Storefront"], operationId: "listAdminReviews", ...editor, responses: { "200": response(anyArray), ...errors([401, 403, 503]) } });
register({ method: "post", path: "/api/admin/reviews", tags: ["Storefront"], operationId: "createAdminReview", ...admin, request: { body: body(reviewInput) }, responses: { "201": response(anyObject), ...errors([400, 401, 403, 503]) } });
register({ method: "patch", path: "/api/admin/reviews/{id}", tags: ["Storefront"], operationId: "updateAdminReview", ...admin, request: { params: pathId, body: body(reviewUpdateInput) }, responses: { "200": response(anyObject), ...errors([400, 401, 403, 404, 503]) } });
register({ method: "delete", path: "/api/admin/reviews/{id}", tags: ["Storefront"], operationId: "deleteAdminReview", ...admin, request: { params: pathId }, responses: { "204": { description: "Review deleted" }, ...errors([401, 403, 404, 503]) } });
...
register({ method: "get", path: "/api/admin/users", tags: ["Internal"], operationId: "listAdminUsers", ...admin, request: { query: userSearchQuery }, responses: { "200": response(anyArray), ...errors([401, 403, 503]) } });
...
register({ method: "post", path: "/api/chat", tags: ["Storefront"], operationId: "chatAssistant", request: { body: body(chatPayloadSchema) }, responses: { "200": response(anyObject), ...errors([400, 500]) } });
...
register({ method: "get", path: "/api/products", tags: ["Storefront"], operationId: "listPublicProducts", request: { query: publicCatalogQuery }, responses: { "200": response(anyObject), ...errors([400, 503]) } });
register({ method: "get", path: "/api/products/{slug}", tags: ["Storefront"], operationId: "getPublicProductDetails", request: { params: pathSlug }, responses: { "200": response(anyObject), ...errors([404, 503]) } });
register({ method: "get", path: "/api/storefront/settings", tags: ["Storefront"], operationId: "getPublicStorefrontSettings", responses: { "200": response(anyObject), ...errors([503]) } });
