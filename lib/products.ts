...
export function mapProduct(record: ProductRecord, source: "database" = "database"): Product {
  ...
  return {
    ...
    reviews: (record.reviews ?? []).map((review) => ({ authorName: review.authorName, rating: review.rating, comfort: review.comfort ?? undefined, quality: review.quality ?? undefined, value: review.value ?? undefined, body: review.body, createdAt: review.createdAt.toISOString(), isFixture: review.isFixture })),
    ...
    purchasable: record.status === "PUBLISHED" && isProductSellable(record.verificationStatus, record.saleStatus) && variants.some((variant) => variant.active && variant.price !== null && variant.price > 0 && variant.stock > 0 && (process.env.NODE_ENV !== "production" || (variant.priceStatus === "VERIFIED" && variant.stockStatus === "VERIFIED"))),
  };
}
